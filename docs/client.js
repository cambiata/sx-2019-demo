...
view_HomeView.prototype = $extend(view_AppBaseView.prototype,{
	...
	,choirAdminView: function() {
		...
		var groupLists = groups.map(function(group1) {
			...
			var groupLists2 = data_SongFilter.Group(group1.name);
			var groupLists3 = data_HomeCell.Songlist(group2,ds__$ImmutableArray_ImmutableArray_$Impl_$.array(groupLists1),[groupLists2]);
			return _gthis.buildCells([groupLists3,data_HomeCell.ListGroupMembers(group1.name),data_HomeCell.InviteGroupMembers(group1.name)]);
		});
		...
		return m.m("section",cells.map(function(cell) {
			...
			case 8:
				return m.m("div.center",[m.m("button.center",{ onclick : function(e) {
					console.log("src/view/HomeView.hx:129:","Click");
					return;
				}},"Gå till butiken")]);
			default:
				return null;
...
view_OverlayView.prototype = $extend(view_AppBaseView.prototype,{
	view: function() {
		if(arguments.length > 0 && arguments[0].tag != this) return arguments[0].tag.view.apply(arguments[0].tag, arguments);
		return new view_SongListView(this.store).view();
	}
...
var view_SongListView = function(store) {
	view_AppBaseView.call(this,store);
};
...
view_SongListView.prototype = $extend(view_AppBaseView.prototype,{
	view: function() {
		if(arguments.length > 0 && arguments[0].tag != this) return arguments[0].tag.view.apply(arguments[0].tag, arguments);
		var songs = data_KorakademinScorxItems.items().filter(function(item) {
			return item.title.indexOf(view_SongListView.searchString) > -1;
		});
		return [m.m("div",[m.m("input[placeholder=Sök titel, upphovsman, besättning]",{ oninput : function(e) {
			return view_SongListView.searchString = e.target.value;
		}, value : view_SongListView.searchString}),m.m("div",view_SongListView.searchString)]),m.m("div",songs.map(function(song) {
			return m.m("div",song.title);
		}))];
	}
