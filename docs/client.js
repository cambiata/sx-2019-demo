...
Client.main = function() {
	...
	m.mount(window.document.querySelector("#playerheader"),new view_OverlayView(store));
};
...
var data_HomeCell = $hxEnums["data.HomeCell"] = { __ename__ : true, __constructs__ : ["Image","Title","Info","Infoblobs","SonglistHeader","Songlist2","Songlist3","SearchChoir","GroupAddSongs","UserAddSongs","ShowFreeSongsIfNothingElse","ListGroupMembers","InviteGroupMembers","AutogeneratedSensusLeaderSonglist","AutogeneratedNewsList","AutogeneratedHistoryList"]
	...
	,Songlist3: ($_=function(title,accesses) { return {_hx_index:6,title:title,accesses:accesses,__enum__:"data.HomeCell",toString:$estr}; },$_.__params__ = ["title","accesses"],$_)
	,SearchChoir: {_hx_index:7,__enum__:"data.HomeCell",toString:$estr}
	,GroupAddSongs: ($_=function(group) { return {_hx_index:8,group:group,__enum__:"data.HomeCell",toString:$estr}; },$_.__params__ = ["group"],$_)
	,UserAddSongs: ($_=function(user) { return {_hx_index:9,user:user,__enum__:"data.HomeCell",toString:$estr}; },$_.__params__ = ["user"],$_)
	,ShowFreeSongsIfNothingElse: ($_=function(user) { return {_hx_index:10,user:user,__enum__:"data.HomeCell",toString:$estr}; },$_.__params__ = ["user"],$_)
	,ListGroupMembers: ($_=function(groupname) { return {_hx_index:11,groupname:groupname,__enum__:"data.HomeCell",toString:$estr}; },$_.__params__ = ["groupname"],$_)
	,InviteGroupMembers: ($_=function(groupname) { return {_hx_index:12,groupname:groupname,__enum__:"data.HomeCell",toString:$estr}; },$_.__params__ = ["groupname"],$_)
	,AutogeneratedSensusLeaderSonglist: {_hx_index:13,__enum__:"data.HomeCell",toString:$estr}
	,AutogeneratedNewsList: {_hx_index:14,__enum__:"data.HomeCell",toString:$estr}
	,AutogeneratedHistoryList: {_hx_index:15,__enum__:"data.HomeCell",toString:$estr}
};
...
view_HomeView.prototype = $extend(view_AppBaseView.prototype,{
	...
	,choirMemberView: function() {
		...
		var groupLists = groups.map(function(group2) {
			...
			return _gthis.buildCells([data_HomeCell.Songlist3(group2.name,accesses)]);
		});
	...
	,choirAdminView: function() {
		...
		var groupLists = groups.map(function(group1) {
			...
			return _gthis.buildCells([data_HomeCell.Songlist3(group1.name,accesses),data_HomeCell.GroupAddSongs(group1),data_HomeCell.InviteGroupMembers(group1.name)]);
		});
	...
	,userSongsView: function() {
		...
		console.log("src/view/HomeView.hx:146:",myListItems);
		return [this.buildCells([data_HomeCell.Title("Mina låtar"),data_HomeCell.Info("Här visas de låtar som du har köpt eller valt genom förmånserbjudanden."),data_HomeCell.Songlist3("Mina låtar",myListItems),data_HomeCell.UserAddSongs(user),data_HomeCell.ShowFreeSongsIfNothingElse(user)])];
	}
	,buildCells: function(cells) {
		...
		return m.m("section",cells.map(function(cell) {
			...
			case 6:
				return m.m("div.centerpad",new view_SongListView3(_gthis.store,cell.title,cell.accesses,onListItemClick).view());
			case 7:
...
var view_SongListView3 = function(store,title,items,onItemClick,cells) {
	view_AppBaseView.call(this,store);
	this.title = title;
	this.items = items.slice();
	this.onItemClick = onItemClick;
...
$hxClasses["view.SongListView3"] = view_SongListView3;
view_SongListView3.__name__ = true;
view_SongListView3.__super__ = view_AppBaseView;
view_SongListView3.prototype = $extend(view_AppBaseView.prototype,{
	view: function() {
		var _gthis = this;
		if(arguments.length > 0 && arguments[0].tag != this) return arguments[0].tag.view.apply(arguments[0].tag, arguments);
		var searchStrings = view_SongListView3.searchString.split(" ");
		if(this.items.length == 0) {
			return m.m("p.graytext","Listan är tom");
		}
		var _g = 0;
		...
		var sort11 = m.m("select",{ onchange : function(e) {
			console.log("src/view/SongListView3.hx:42:",e.target.selectedIndex);
			return view_SongListView3.sortindex = e.target.selectedIndex;
		}, onblur : function(e1) {
			console.log("src/view/SongListView3.hx:46:",e1.target.selectedIndex);
			return view_SongListView3.sortindex = e1.target.selectedIndex;
		}},sort1);
		this.items.sort(function(a,b) {
			switch(view_SongListView3.sortindex) {
			case 0:
				return Reflect.compare(a.song.title,b.song.title);
			case 1:
				return Reflect.compare(a.song.composer,b.song.composer);
		...
		var songlist = m.m("div.scorxlist",this.items.map(function(item1) {
			...
			return m.m("div.scorxitem",[songlist1,m.m("div.columnTwo",m.m("div.scorxaccess",[m.m("p.smalltext","" + $hxEnums[e2.__enum__].__constructs__[e2._hx_index]),m.m("p.smalltext","" + Std.string(Type.enumParameters(item1.access)))])),m.m("div.columnThree",[m.m("button.round",{ onclick : function(e3) {
				...
				return;
			}},"Play")]),_gthis.cells != null && _gthis.cells != [] ? new view_HomeView(_gthis.store).buildCells(_gthis.cells) : null]);
		}));
		return m.m("details[open=true].songlist",[m.m("summary",m.m("span.songlisttitle",this.title)),m.m("div.songlistcontent",[m.m("div.searchinput",[m.m("input[placeholder=Sök titel, upphovspersoner, besättning]",{ oninput : function(e4) {
			return view_SongListView3.searchString = e4.target.value;
		}, value : view_SongListView3.searchString}),sort11,m.m("span"," " + this.items.length + " låtar i listan")]),songlist])]);
	}
	,__class__: view_SongListView3
});
...
view_SongListView3.searchString = "";
view_SongListView3.sortindex = 0;
